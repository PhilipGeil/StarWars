...
namespace Starwars
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            AssignmentOne(planets);
            Console.WriteLine();
            AssignmentTwo(planets);
            Console.WriteLine();
            AssignmentThree(planets);
            Console.WriteLine();
            AssignmentFour(planets);
            Console.WriteLine();
            AssignmentFive(planets);
            Console.WriteLine();
            AssignmentSix(planets);
            Console.WriteLine();
            AssignmentSeven(planets);
            Console.WriteLine();
            AssignmentEight(planets);
            Console.WriteLine();
            AssignmentNine(planets);
            Console.WriteLine();
            AssignmentTen(planets);
            Console.WriteLine();
            AssignmentElleven(planets);
            Console.WriteLine();
            AssignmentTwelve(planets);
            Console.WriteLine();
            AssignmentThirteen(planets);
            Console.WriteLine();
            AssignmentFourteen(planets);
            Console.WriteLine();
            AssignmentFifteen(planets);
            Console.WriteLine();
            AssignmentSixteen(planets);
            Console.ReadKey();
        }

        static void AssignmentOne(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.Name.StartsWith("M"));
            Console.WriteLine("Assignment One");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }

        static void AssignmentTwo(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.Name.ToLower().Contains("y"));
        ...
        static void AssignmentThree(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.Name.Length > 9).Where(p => p.Name.Length < 15);
        ...
        static void AssignmentFour(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.Name.IndexOf("a") == 1).Where(p => p.Name.EndsWith("e"));
        ...
        static void AssignmentFive(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.RotationPeriod > 40).OrderBy(p => p.RotationPeriod);
            Console.WriteLine("Assignment Five");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentSix(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.RotationPeriod > 10).Where(p => p.RotationPeriod < 20).OrderBy(p => p.Name);
            Console.WriteLine("Assignment Six");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentSeven(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.RotationPeriod > 30).OrderBy(p => p.Name).ThenBy(p => p.RotationPeriod);
            Console.WriteLine("Assignment Seven");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentEight(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.RotationPeriod < 30 || p.SurfaceWater > 50).Where(p => p.Name.ToLower().Contains("ba")).OrderBy(p => p.Name).ThenBy(p => p.SurfaceWater).ThenBy(p => p.RotationPeriod);
            Console.WriteLine("Assignment Eight");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentNine(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.SurfaceWater > 0).OrderByDescending(p => p.SurfaceWater);
            Console.WriteLine("Assignment Nine");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentTen(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.Diameter != 0).Where(p => p.Population != 0).OrderBy(p => ((4 * Math.PI * Math.Pow(p.Diameter / 2, 2)) / p.Population));
            Console.WriteLine("Assignment Ten");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentElleven(List<Planet> planets)
        {
            NameComparer nameComparer = new NameComparer();
            var _planets = planets.Except(planets.Where(p => p.RotationPeriod > 0), nameComparer);
            Console.WriteLine("Assignment Elleven");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentTwelve(List<Planet> planets)
        {
            NameComparer nameComparer = new NameComparer();
            var _planets = planets.Where(p => p.Name.ToLower().StartsWith("a") || p.Name.ToLower().EndsWith("s")).Union(planets.Where(p => p.Terrain != null && p.Terrain.Contains("rainforests")), nameComparer);
            Console.WriteLine("Assignment Twelve");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentThirteen(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.Terrain != null && p.Terrain.Contains("deserts"));
            Console.WriteLine("Assignment Thirteen");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentFourteen(List<Planet> planets)
        {
            var _planets = planets.Where(p => p.Terrain != null && p.Terrain.Contains("swamps")).OrderBy(p => p.RotationPeriod).ThenBy(p => p.Name);
            Console.WriteLine("Assignment Fourteen");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentFifteen(List<Planet> planets)
        {
            var _planets = planets.Where(p => Regex.Match(p.Name, @"([aeiouy])\1", RegexOptions.IgnoreCase).Success);
            Console.WriteLine("Assignment Fifteen");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        }
        static void AssignmentSixteen(List<Planet> planets)
        {
            var _planets = planets.Where(p => Regex.Match(p.Name, @"([klrn])\1", RegexOptions.IgnoreCase).Success).OrderByDescending(p => p.Name);
            Console.WriteLine("Assignment Sixteen");
            foreach (Planet item in _planets)
            {
                Console.WriteLine(item.Name);
            }
        ...
        static List<Planet> LoadData()
        {
            List<Planet> planets = new List<Planet>()
            {
                ...
                new Planet { Name="Troiken",Terrain= new List<string>{ "deserts", "tundra","rainforests","mountains"} },
